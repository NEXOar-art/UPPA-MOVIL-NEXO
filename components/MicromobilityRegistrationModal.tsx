...
const MicromobilityRegistrationModal: React.FC<MicromobilityRegistrationModalProps> = ({ currentUser, userLocation, onSubmit, onClose }) => {
  ...
  // FIX: canSubmit ahora es más robusto y permite el registro si hay ubicación fijada
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 text-gray-200 pb-6">
      <div className="bg-cyan-500/10 p-3 rounded-lg border border-cyan-500/30 text-[10px] text-slate-300">
          ...
          Regístrate para aparecer en el Nexo Urbano.
      </div>

      <div className="space-y-4">
        <div>
            <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Nombre de Unidad</label>
            <input
              type="text" value={serviceName} onChange={(e) => setServiceName(e.target.value)}
              className="w-full p-3 ps-input bg-black/40 border-white/10" placeholder="Ej: Moto-Relámpago" required
            />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">WhatsApp</label>
                <input
        ...
        <div className="grid grid-cols-2 gap-4">
            <input
                ...
                className="w-full p-3 ps-input bg-black/40 border-white/10" placeholder="Modelo" required
            />
            <input
      ...
      <div className="bg-slate-800/40 p-4 rounded-xl border border-white/5 space-y-4">
        <button
            ...
            className={`w-full ps-button active flex items-center justify-center space-x-3 py-3 ${location ? 'border-green-500 text-green-300' : ''}`}
        >
            {isFetchingLocation ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-location-crosshairs"></i>}
            <span>{location ? 'POSICIÓN FIJADA' : 'FIJAR MI BASE (GPS)'}</span>
        </button>

        <div className="flex items-center gap-2">
            <input
                type="text" value={manualAddress} onChange={(e) => setManualAddress(e.target.value)}
                placeholder="O busca dirección..." className="flex-grow ps-input text-xs py-2 bg-black/20"
            />
            <button type="button" onClick={handleManualSearch} className="ps-button p-2.5"><i className="fas fa-search"></i></button>
        </div>
        {address && <p className="text-[9px] text-cyan-500 truncate font-mono italic">{address}</p>}
      </div>
      
      <div className="space-y-3">
        <label className="block text-[10px] font-bold text-slate-500 uppercase">Abono de Visibilidad</label>
            ...
            {Object.entries(MICROMOBILITY_PRICING[type]).map(([hours, price]) => (
                 <button
                    ...
                    className={`flex flex-col items-center justify-center p-2 rounded-lg border-2 transition-all 
                                ${subscriptionDurationHours === Number(hours) ? 'bg-cyan-500/20 border-cyan-500' : 'bg-slate-900 border-white/5 opacity-50'}`
                    }
                >
                    <span className="text-xs font-black">{hours}H</span>
                    <span className="text-[8px] text-yellow-400">${price}</span>
                </button>
            ))}
        </div>
      </div>

      <div className="flex items-center gap-3 px-1 bg-black/20 p-3 rounded-lg">
          <input 
              type="checkbox" id="termsAccepted" checked={termsAccepted} 
              onChange={(e) => setTermsAccepted(e.target.checked)}
              className="w-4 h-4 rounded bg-black/40 border-white/10 text-cyan-500"
          />
          <label htmlFor="termsAccepted" className="text-[10px] font-bold text-slate-300 cursor-pointer uppercase">
              Acepto los términos de responsabilidad
          </label>
      </div>

      <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
        <button type="button" onClick={onClose} className="px-6 py-2.5 text-xs text-slate-500">Cerrar</button>
        <button
          type="submit" disabled={!canSubmit}
          className="ps-button active px-10 py-2.5 text-xs flex items-center justify-center shadow-lg disabled:opacity-30"
        >
          {isSubmitting ? <LoadingSpinner size="w-4 h-4" /> : <><i className="fas fa-fighter-jet mr-2"></i> INICIAR MISIÓN</>}
        </button>
      </div>
    </form>
  );
